...
<template>
    ...
    <ChatLayout>
        <template #aside>
            ...
            <ul class="p-2">
                <li v-if="chat"
                    class="px-4 py-2 my-2 flex justify-between font-semibold text-green-400 bg-slate-900 hover:bg-slate-700 rounded-lg durtion-200">
                ...
                <template v-for="message in messages" :key="message.id">
                    <li
                        :class="[message.id == chat?.id ? 'bg-slate-700' : 'bg-slate-900', 'px-4 py-2 my-2 flex justify-between font-semibold text-slate-400 hover:bg-slate-700 rounded-lg durtion-200']">
                        ...
                        <div v-if="message.id == chat?.id">
                            <button @click="showDeleteButton = !showDeleteButton" v-show="!showDeleteButton">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            ...
                            <span v-show="showDeleteButton" class="flex justify-between">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-green-300 hover:text-green-700">
                                ...
                                <button @click="showDeleteButton = false">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-red-300 hover:text-red-800">
        ...
        <!-- default slot . messages and all history  -->
        ...
        <!--  where you can write a Q -->
        <template #form>
            <section class="px-6 top-0">
                <div class="w-full">
                    <div class="relative flex-1 flex items center">
                        <input type="text" ref="promtInput" v-model="form.promt" :disabled="form.processing"
                            @keyup.enter="submit" class="w-full bg-slate-700 text-white rounded-lg"
                        ...
                        <div class="absolute inset-y-0 right-0 flex items-center pl-3">
                            <svg v-if="!form.processing" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.5" stroke="currentColor" class="w-6 h-6 -m-8 text-slate-200">
                                <path stroke-linecap="round" stroke-linejoin="round"
